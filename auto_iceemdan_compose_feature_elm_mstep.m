...
%划分训练集和测试集
step=1
nn=251+step;       %设置验证集个数 nn=251+x x为真实世界中的x步预测
n=length(shuru)-nn ; 
%训练集
x_train = shuru(1:n,:)';            
%测试集-251个样本
...
%以下为重构高频、低频和趋势项
    ...
    mmmm_dipin=modesf(7,:)
    for i=8:number_imf              %number_imf-1  %%20231210
        mmmm_dipin=mmmm_dipin+modesf(i,:);
    end;
    
    %for i=5:number_imf-2   
        %mmmm_zhongpin=mmmm_zhongpin+modesf(i,:);
    %end;
    %mmmm_dipin=modesf((number_imf-1),:)+modesf(number_imf,:)
        
    modesf=[] 
    modesf=[mmmm_imf1', mmmm_imf2', mmmm_imf3', mmmm_imf4', mmmm_imf5', mmmm_imf6', mmmm_dipin']  %mmmm_imf6', 
    modesf=modesf'
    ...
    for i=1:number_imf2
        ...
        if i == 1
            NIR=[IMF_shuru1',IMF_shuru2',IMF_shuru3',IMF_shuru4',IMF_shuru5',IMF_shuru6',IMF_shuru7',IMF_shuru8']
            num=10
        end;  
        if i == 2
        ...
        if i == 4
            NIR=[IMF_shuru1',IMF_shuru2',IMF_shuru3',IMF_shuru4']
            num=10
        end;  
        if i == 5
            NIR=[IMF_shuru1',IMF_shuru2',IMF_shuru3',IMF_shuru4',IMF_shuru5']
...
%%4.ELM训练
        [IW1,B1,H1,TF1,TYPE1] = elmtrain_regularization(Pn_train,Tn_train,num,'sig',0,0.00001);
%%5.ELM仿真测试
